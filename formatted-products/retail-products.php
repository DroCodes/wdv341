...
<body>
    ...
    <section>
        ...
        while ($row = $stmt->fetch(PDO::FETCH_ASSOC)) {
            ...
            // Check if there is product_status data in the record
            if (!empty($row['product_status'])) {
                echo '<p class="productStatus">' . $row['product_status'] . '</p>';
            }

            // Check if product_inventory is less than 10 and apply productLowInventory class if necessary
            $inventory = $row['product_inStock'];
            $inventoryClass = ($inventory < 10) ? 'productLowInventory' : '';
            echo '<p class="productInventory ' . $inventoryClass . '">In Stock: ' . $inventory . '</p>';
